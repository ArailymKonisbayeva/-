...
(function(){
  ...
  function renderGamePage(workId,gameId){
    ...
    activeGame={workId,gameId,index:0,score:0,answers:[],order:null,done:false,nodeId:game.storyQuest?.startNode||null,history:[],pendingChoice:null};
    root().innerHTML=`<section class="ds-game-shell"><div class="ds-game-nav"><button class="back" onclick="NationalGames.chooseLiteraryGame('${game.mechanic}')">← Артқа</button><button class="ds-end-game" onclick="endVerifiedGame()">Аяқтау</button></div><div class="ds-game-top"><div><span class="ds-kicker">${esc(work.title)}</span><h1>${esc(game.title)} — ${esc(work.title)}</h1><p class="muted">${esc(game.instruction)}</p></div><span class="tag">${esc(game.difficulty)}</span></div><div class="ds-fact-note"><b>Оқу мақсаты:</b> ${esc(game.learningObjective)}</div><div id="gameSession"></div></section>`;
  ...
  function renderGameStep(){
    ...
    if(game.type==='branch')return renderBranchGame(work,game,box);
  ...
  function questCover(work){return workCovers[work.id]||'assets/asyk/asyk-village-intro.png'}
  function renderQuestPath(){return `<div class="sq-path" aria-label="Өткен шешімдер"><span class="active"></span>${activeGame.history.map((item,index)=>`<i></i><span class="done" title="${esc(item.text)}">${index+1}</span>`).join('')}</div>`}
  function renderBranchGame(work,game,box){
    const quest=game.storyQuest;if(!quest)return;
    const ending=quest.endings[activeGame.nodeId];if(ending)return renderQuestEnding(work,game,quest,ending,box);
    const node=quest.nodes[activeGame.nodeId],pending=activeGame.pendingChoice;if(!node)return;
    box.innerHTML=`<section class="sq-shell" style="--sq-image:url('${questCover(work)}')"><header class="sq-scene"><div class="sq-scene-top"><span>${esc(work.title)} · интерактивті әдеби квест</span><b>Шешім ${activeGame.history.length+1} / 3</b></div><div class="sq-scene-copy"><span class="sq-label">${esc(node.label)}</span><small>${esc(node.character)}</small><h2>${esc(node.title)}</h2><p>${esc(node.situation)}</p></div>${renderQuestPath()}</header><div class="sq-panel ${pending?'has-consequence':''}">${pending?`<article class="sq-consequence"><span>Таңдаудың салдары</span><h3>${esc(pending.text)}</h3><div class="sq-impact-grid"><p><b>Не болды?</b>${esc(pending.consequence.result)}</p><p><b>Кейіпкер</b>${esc(pending.consequence.emotion)}</p><p><b>Қарым-қатынас</b>${esc(pending.consequence.relations)}</p><p><b>Сюжет пен мағына</b>${esc(pending.consequence.direction)} ${esc(pending.consequence.meaning)}</p></div><button class="primary" onclick="nextStoryBranch()">Жалғастыру →</button></article>`:`<div class="sq-choice-head"><div><span>ТАҢДАУ</span><h3>${esc(node.character)} не істейді?</h3></div><small>Дұрыс не қате жауап жоқ. Әр шешім бөлек жол ашады.</small></div><div class="sq-choices">${node.choices.map((choice,index)=>`<button class="sq-choice" onclick="chooseStoryBranch(${index})"><i>${esc(choice.icon||'→')}</i><span><b>${esc(choice.text)}</b><small>${esc(choice.hint||'')}</small></span><em>→</em></button>`).join('')}</div>`}</div></section>`;
  }
  window.chooseStoryBranch=function(index){if(!activeGame||activeGame.pendingChoice)return;const {game}=gameByIds(activeGame.workId,activeGame.gameId),node=game.storyQuest?.nodes[activeGame.nodeId],choice=node?.choices[index];if(!choice)return;activeGame.pendingChoice=choice;activeGame.history.push({nodeId:activeGame.nodeId,choiceId:choice.id,text:choice.text,consequence:choice.consequence,next:choice.next});activeGame.score=activeGame.history.length;renderGameStep()};
  window.nextStoryBranch=function(){if(!activeGame?.pendingChoice)return;activeGame.nodeId=activeGame.pendingChoice.next;activeGame.pendingChoice=null;activeGame.index=activeGame.history.length;renderGameStep();window.scrollTo({top:0,behavior:'smooth'})};
  window.restartStoryQuest=function(){if(!activeGame)return;const {game}=gameByIds(activeGame.workId,activeGame.gameId);activeGame.index=0;activeGame.score=0;activeGame.answers=[];activeGame.nodeId=game.storyQuest?.startNode||null;activeGame.history=[];activeGame.pendingChoice=null;renderGameStep();window.scrollTo({top:0,behavior:'smooth'})};
  function renderQuestEnding(work,game,quest,ending,box){
    const path=activeGame.history.map((item,index)=>`<li><span>${index+1}</span><div><b>${esc(item.text)}</b><small>${esc(item.consequence.result)}</small></div></li>`).join('');
    box.innerHTML=`<section class="sq-ending"><header><span>${esc(ending.type)}</span><h2>${esc(ending.title)}</h2><p>${esc(ending.summary)}</p></header><div class="sq-ending-grid"><article><h3>Сенің таңдаған жолың</h3><ol>${path}</ol></article><article><h3>Бұл шешімдер неге әкелді?</h3><p><b>Кейіпкер:</b> ${esc(ending.heroEffect)}</p><p><b>Басқалар:</b> ${esc(ending.othersEffect)}</p><p><b>Сюжет:</b> ${esc(ending.plotEffect)}</p></article><article class="sq-compare"><h3>Түпнұсқамен салыстыру</h3><p><b>Түпнұсқада:</b> ${esc(quest.original)}</p><p><b>Сенің нұсқаңда:</b> ${esc(ending.comparison)}</p></article><article class="sq-meaning"><h3>Неліктен бұл маңызды?</h3><p>${esc(ending.ideaEffect)}</p><small>Бұл талдау автордың өмірбаяндық ниеті туралы болжам емес; ол шығармадағы шешім мен салдардың байланысын көрсетеді.</small></article></div><div class="sq-ending-actions"><button class="primary" onclick="restartStoryQuest()">Қайта бастау</button><button class="secondary" onclick="NationalGames.chooseLiteraryGame('branch')">Басқа шығарманы таңдау</button><button class="secondary" data-go="games">Ойындар орталығы</button></div></section>`;
    const total=activeGame.history.length,rows=gameResults();rows.unshift({workId:work.id,gameId:game.id,score:total,total,percent:100,date:new Date().toISOString(),ending:ending.id,path:activeGame.history.map(item=>item.choiceId),learningObjective:game.learningObjective});store.write('adebiet-game-results',rows.slice(0,100));
  }
  ...
  function finishVerifiedGame(){
    ...
    const {work,game}=gameByIds(activeGame.workId,activeGame.gameId),total=game.type==='order'?1:game.type==='match'?game.pairs.length:game.type==='branch'?Math.max(1,activeGame.history.length):game.questions.length,percent=Math.round(activeGame.score/total*100),review=[...new Set(activeGame.answers.filter(item=>!item.correct).map(item=>item.topic))];
    ...
    document.querySelector('#gameSession').innerHTML=`<article class="ds-question ds-result"><span class="ds-kicker">Ойын аяқталды</span><div class="ds-result-score">${percent}%</div><h2>${activeGame.score} / ${total} ${game.type==='branch'?'таңдау':'дұрыс'}</h2><p>${game.type==='branch'?'Сен шығарма тақырыптарына сүйенген жеке балама желі құрдың. Бұл нұсқа түпнұсқа сюжет емес.':percent>=80?'Материал жақсы меңгерілген.':percent>=50?'Жақсы бастама. Қате жауаптарды қайта қарап, тағы бір рет байқап көріңіз.':'Бұл — үйренудің бір бөлігі. Шығарма бөлімдерін қайта қарап, асықпай қайталаңыз.'}</p>${review.length?`<div class="ds-fact-note"><b>Қайталау ұсынылады:</b> ${review.map(esc).join(', ')}</div>`:''}<div class="actions" style="justify-content:center"><button class="primary" onclick="openGame('${work.id}','${game.id}')">Қайта ойнау</button><button class="secondary" onclick="nextLiteraryGame('${work.id}','${game.mechanic}')">Келесі</button><button class="secondary" onclick="NationalGames.chooseLiteraryGame('${game.mechanic}')">Басқа шығарма таңдау</button></div></article>`;
  }
  window.nextLiteraryGame=function(workId,mechanic){const work=byId(workId),games=work?.games||[],index=games.findIndex(game=>game.mechanic===mechanic),next=games[(index+1)%games.length];if(next)openGame(workId,next.id)};
  ...
  function handleRoute(){
    ...
    document.body.classList.toggle('assistant-route',route==='assistant');
    if(route==='map'||route==='author')return syncNav('map');
    if(route==='work')return renderWork(parts[1]);
    if(route==='games'&&parts[1]==='literary'&&parts[2])return window.NationalGames?.renderLiteraryWorkPicker(parts[2]);
