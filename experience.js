...
(function(){
  ...
  function makeWorkCard(work){
    ...
    return `<article class="work-card">${favoriteButton(work.id)}<span class="tag">${esc(work.genre||'Дерек нақтылануда')}</span><h3>${esc(work.title)}</h3><div class="muted">${esc(work.author)}</div><p class="muted">${esc(work.intro||'Бұл шығарма туралы тексерілген мәлімет толықтырылуда.')}</p><div class="ds-chip-list">${done?'<span class="ds-chip">✓ Зерттелді</span>':''}<span class="ds-chip">${statusLabel(work)}</span></div><button class="small-btn" style="margin-top:auto" onclick="openWork('${work.id}')">Зерттеу →</button></article>`;
  }
  ...
  function renderHome(){
    ...
    const featured=['kokserak','alpan','akbilek'].map(byId).filter(Boolean);
    root().innerHTML=`<section class="ds-hero"><div><span class="ds-kicker">Қазақ әдебиетінің танымдық кеңістігі</span><h1>Оқы. Түсін.<br><span>Байланысын көр.</span></h1><p class="lead">Қазақстан өңірлерін, шығармаларды және кейіпкерлер әлемін дерекке сүйеніп зерттеңіз. Біліміңізді мазмұнға сай ойындармен бекітіңіз.</p><div class="actions"><button class="primary" data-go="map">Әдеби Қазақстанды ашу</button><button class="secondary" data-go="library">Шығармаларды зерттеу</button></div><div class="ds-hero-stats"><div class="ds-mini-stat"><b>13</b><span>негізгі шығарма</span></div><div class="ds-mini-stat"><b>20</b><span>әкімшілік өңір</span></div><div class="ds-mini-stat"><b>${done}</b><span>зерттелген шығарма</span></div></div></div><div class="ds-hero-visual" aria-label="Әдеби саяхаттың көркем композициясы"><div class="ds-hero-quote"><span class="ds-kicker">Білім жолы</span><p>Мәтінді жаттау емес, шығарманың ішкі байланысын түсіну.</p><small>${played?`${played} ойын нәтижесі сақталған`:'Алғашқы шығарманы таңдап, саяхатты бастаңыз'}</small></div></div></section><section class="section"><div class="section-head"><div><h2>Саяхаттың төрт қадамы</h2><p>Әр бөлімнің нақты оқу мақсаты бар</p></div></div><div class="ds-grid"><article class="ds-card sage"><div class="ds-card-icon">⌖</div><h3>Өңірді зерттеу</h3><p>Карта арқылы қаламгердің туған өлкесі мен мәдени ортасын көріңіз.</p></article><article class="ds-card cream"><div class="ds-card-icon">▤</div><h3>Шығарманы түсіну</h3><p>Кейіпкерлер, сюжет, тақырып және байланыстарды бір жерден қараңыз.</p></article><article class="ds-card blue"><div class="ds-card-icon">◇</div><h3>Білімді бекіту</h3><p>Тек тексерілген деректен құрылған мазмұндық ойындарды орындаңыз.</p></article></div></section><section class="section"><div class="section-head"><div><h2>Зерттеуді бастау</h2><p>Дерекқордағы негізгі шығармалар</p></div><button class="link" data-go="library">Барлығын көру →</button></div><div class="cards">${featured.map(makeWorkCard).join('')}</div></section>`;
  ...
  function renderOrderGame(work,game,box){
    ...
    box.innerHTML=`<div class="ds-progress-label"><span>1 тапсырма</span><span>Оқиғаларды реттеңіз</span></div><article class="ds-question"><h2>${esc(game.prompt||'Оқиғаларды дұрыс ретке қойыңыз')}</h2><div class="ds-order-list">${activeGame.order.map((item,index)=>`<div class="ds-order-item"><span class="ds-event-index">${index+1}</span><span>${esc(item.text)}</span><span class="ds-order-controls"><button onclick="moveOrderItem(${index},-1)" aria-label="Жоғары жылжыту">↑</button><button onclick="moveOrderItem(${index},1)" aria-label="Төмен жылжыту">↓</button></span></div>`).join('')}</div><button class="primary" style="margin-top:17px" onclick="checkOrderGame()">Ретін тексеру</button><div id="gameFeedback"></div></article>`;
  }
  ...
  function renderMatchGame(work,game,box){
    const options=game.pairs.map(pair=>pair.right).sort((a,b)=>a.localeCompare(b,'kk'));
    box.innerHTML=`<article class="ds-question"><h2>${esc(game.prompt||'Сәйкес жұптарды табыңыз')}</h2><div class="match-grid">${game.pairs.map((pair,index)=>`<div><b>${esc(pair.left)}</b></div><select data-match="${index}"><option value="">Сәйкестігін таңдаңыз</option>${options.map(option=>`<option value="${esc(option)}">${esc(option)}</option>`).join('')}</select>`).join('')}</div><button class="primary" style="margin-top:17px" onclick="checkMatchGame()">Жауаптарды тексеру</button><div id="gameFeedback"></div></article>`;
  }
  ...
  function renderAssistant(workId){
    ...
    root().innerHTML=`<section class="ds-page"><div class="ds-page-head"><div><span class="ds-kicker">Дерекке негізделген көмек</span><h1>AI әдеби көмекші</h1><p>${work?`Ағымдағы контекст: «${esc(work.title)}». Жауап тек жобаның тексерілген өрістерінен құрылады.`:'Шығарманы таңдаңыз. Дерек жетіспесе, көмекші оны ашық айтады.'}</p></div></div><section class="assistant-layout"><aside class="assistant-card"><h3>Шығарма контексті</h3><select id="assistantWork" class="search-input" onchange="changeAssistantWork(this.value)"><option value="">Шығарманы таңдаңыз</option>${list().map(item=>`<option value="${item.id}" ${work?.id===item.id?'selected':''}>${esc(item.title)}</option>`).join('')}</select><h3 style="margin-top:22px">Жылдам сұрақтар</h3><button class="quick" onclick="askVerifiedAssistant('author')">Авторы мен жанры →</button><button class="quick" onclick="askVerifiedAssistant('characters')">Кейіпкерлері →</button><button class="quick" onclick="askVerifiedAssistant('plot')">Сюжеті →</button><button class="quick" onclick="askVerifiedAssistant('themes')">Тақырыптары →</button><button class="quick" onclick="askVerifiedAssistant('relations')">Байланыстары →</button><button class="quick" onclick="askVerifiedAssistant('vocabulary')">Сөздігі →</button></aside><div class="chat"><div class="chat-head">◇ Әдеби көмекші <span class="muted">· verified context</span></div><div class="messages" id="messages"><div class="bubble bot">Сәлем! Мен шығарма туралы тек тексерілген дерекке сүйеніп жауап беремін. Факт пен әдеби интерпретацияны бөлек көрсетемін.</div></div><form class="chat-form" onsubmit="askVerifiedAssistant('input');return false"><input id="chatInput" placeholder="Мысалы: Көксеректің кейіпкерлері кім?" autocomplete="off"><button>Жіберу</button></form></div></section></section>`;
